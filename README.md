...
```bash
conda install -c pmuguda snap-gpt          # newest built SNAP
conda install -c pmuguda snap-gpt=11.0     # a specific SNAP version
gpt -h
```

`snap-gpt` repackages the official ESA SNAP distribution into a conda package that
ships **only the headless `gpt`** command plus the **full SAR stack** — every SAR
sensor and all SAR functionality SNAP offers — using this conda environment's own
`openjdk` as the JVM.
...
`9.0` · `10.0` · `11.0` · `12.0` · `13.0` — pick with `snap-gpt=<version>`.
...
auto-detects the installation by finding the `snap` launcher on `PATH`. `snap-gpt`
preserves the standard SNAP install layout (`bin/snap`, `bin/gpt`, `etc/*.properties`)
and puts the real binaries on `PATH`, so pyroSAR detects it with **zero configuration**:
...
from-source feedstock is a possible future v2. Until then: the `-c pmuguda` channel.
